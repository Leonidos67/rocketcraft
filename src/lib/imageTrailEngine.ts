...
class ImageTrailVariant2 extends ImageTrailBase {
  ...
  showNextImage() {
    ...
    this.imgPosition = this.imgPosition < this.imagesTotal - 1 ? this.imgPosition + 1 : 0;
    const img = this.images[this.imgPosition];
    if (!img.rect || !img.DOM.inner) return;
...
const variantMap: Record<ImageTrailVariant, new (container: HTMLElement) => ImageTrailBase> = {
...
export function createImageTrail(container: HTMLElement, variant: ImageTrailVariant = 1) {
