...
/** Smooth multi-octave value noise (Perlin-like feel without CDN). */
function createNoiseTexture(size = 256): THREE.DataTexture {
  const grid = 32;
  const cells = new Float32Array((grid + 1) * (grid + 1));
  let seed = 0x9e3779b9;
  const rand = () => {
    seed |= 0;
  ...
  const sampleGrid = (x: number, y: number) => {
    const x0 = Math.floor(x);
    const y0 = Math.floor(y);
    const fx = smoothstep(x - x0);
    const fy = smoothstep(y - y0);
    const i00 = y0 * (grid + 1) + x0;
    const i10 = i00 + 1;
    const i01 = i00 + (grid + 1);
    const i11 = i01 + 1;
    const a = cells[i00] * (1 - fx) + cells[i10] * fx;
    const b = cells[i01] * (1 - fx) + cells[i11] * fx;
    return a * (1 - fy) + b * fy;
  };
  ...
  const fbm = (x: number, y: number) => {
    ...
    let freq = 1;
    for (let o = 0; o < 4; o += 1) {
      value += sampleGrid(((x * freq) % grid + grid) % grid, ((y * freq) % grid + grid) % grid) * amp;
      freq *= 2;
      amp *= 0.5;
    }
    return value;
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      const n = fbm((x / size) * grid, (y / size) * grid);
...
export function Orb({
  ...
  return (
    <div
      className={`${className ?? 'relative h-full w-full'} animate-in fade-in zoom-in-95 duration-700 fill-mode-forwards`}
    >
      <Canvas
...
function Scene({
  ...
  const perlinNoiseTexture = useMemo(() => createNoiseTexture(256), []);
  ...
  useFrame((_, delta: number) => {
    ...
    if (u.uOpacity.value < 1) {
      u.uOpacity.value = Math.min(1, u.uOpacity.value + dt * 0.55);
    }
    ...
    if (modeRef.current === 'manual') {
      ...
      const t = u.uTime.value * 1.4;
      if (agentRef.current === null) {
        targetIn = 0.08 + 0.04 * Math.sin(t * 0.55);
        targetOut = 0.28 + 0.05 * Math.sin(t * 0.4 + 0.8);
      } else if (agentRef.current === 'listening') {
        targetIn = clamp01(0.5 + Math.sin(t * 2.4) * 0.22);
        targetOut = 0.42;
      } else if (agentRef.current === 'talking') {
        targetIn = clamp01(0.58 + Math.sin(t * 3.2) * 0.16);
        targetOut = clamp01(0.68 + Math.sin(t * 2.6) * 0.16);
      } else {
        const base = 0.34 + 0.06 * Math.sin(t * 0.55);
        const wander = 0.04 * Math.sin(t * 1.5) * Math.sin(t * 0.3 + 1.2);
        targetIn = clamp01(base + wander);
        targetOut = clamp01(0.44 + 0.1 * Math.sin(t * 0.85 + 0.6));
      }
    }

    const ease = 1 - Math.exp(-dt * 4.2);
    curInRef.current += (targetIn - curInRef.current) * ease;
    curOutRef.current += (targetOut - curOutRef.current) * ease;

    const targetSpeed = 0.08 + (1 - Math.pow(curOutRef.current - 1, 2)) * 0.55;
    animSpeedRef.current += (targetSpeed - animSpeedRef.current) * ease;

    u.uAnimation.value += dt * animSpeedRef.current;
    u.uInputVolume.value = curInRef.current;
    u.uOutputVolume.value = curOutRef.current;
    u.uColor1.value.lerp(targetColor1Ref.current, ease * 0.65);
    u.uColor2.value.lerp(targetColor2Ref.current, ease * 0.65);
  });
