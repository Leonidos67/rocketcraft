...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let next: string | undefined = config[key] as string | undefined;
      const cap = key.charAt(0).toUpperCase() + key.slice(1);
      const configRecord = config as Record<string, string | undefined>;
      if (width <= 480 && configRecord[`mobile${cap}`]) next = configRecord[`mobile${cap}`];
      else if (width <= 768 && configRecord[`tablet${cap}`]) next = configRecord[`tablet${cap}`];
      else if (width <= 1024 && configRecord[`desktop${cap}`]) next = configRecord[`desktop${cap}`];
      setValue(next);
    };
...
(GradualBlurMemo as typeof GradualBlurMemo & { PRESETS: typeof PRESETS }).PRESETS = PRESETS;
(
  GradualBlurMemo as typeof GradualBlurMemo & { CURVE_FUNCTIONS: typeof CURVE_FUNCTIONS }
