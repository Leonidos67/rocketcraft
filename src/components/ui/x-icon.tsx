...
const XIcon = forwardRef<XIconHandle, XIconProps>(
  (
    ...
    }, [path1Controls, path2Controls, reduced, svgControls]);

    const runNormal = useCallback(() => {
      svgControls.start('normal');
      path1Controls.start('normal');
      path2Controls.start('normal');
    }, [path1Controls, path2Controls, svgControls]);

    useImperativeHandle(ref, () => {
      isControlled.current = true;
      return {
        startAnimation: runAnimate,
        stopAnimation: runNormal,
      };
    });
    ...
    const handleEnter = useCallback(
      (e?: MouseEvent<HTMLDivElement>) => {
        ...
        if (!isControlled.current) runAnimate();
        else onMouseEnter?.(e);
      },
      [isAnimated, reduced, runAnimate, onMouseEnter]
    );
